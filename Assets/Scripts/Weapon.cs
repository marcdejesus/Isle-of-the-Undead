...
namespace InfimaGames.LowPolyShooterPack
{
    ...
    public class Weapon : WeaponBehaviour
    {
        ...
        public override void Fire(float spreadMultiplier = 1.0f)
        {
            ...
            {
                shootDirection = (hit.point - muzzleSocket.position).normalized;

                // Check if the hit object has an Enemy script.
                Enemy enemy = hit.collider.GetComponent<Enemy>();
                if (enemy != null)
                {
                    enemy.TakeDamage(25); // Adjust damage value as needed.
                }
            }

            // Spawn projectile for visual representation.
            GameObject projectile = Instantiate(prefabProjectile, muzzleSocket.position, Quaternion.LookRotation(shootDirection));
